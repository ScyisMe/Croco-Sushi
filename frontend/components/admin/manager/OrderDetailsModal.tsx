...
export default function OrderDetailsModal({
    ...
    // Fetch full order details to get items and history
    const { data: fullOrder, isLoading } = useQuery({
        queryKey: ['admin_order', initialOrder?.id],
        queryFn: async () => {
            if (!initialOrder?.id) return null;
            const response = await apiClient.get<ManagerOrder>(`/admin/orders/${initialOrder.id}`);
